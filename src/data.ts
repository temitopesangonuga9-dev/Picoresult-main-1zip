...
// Domain is derived from the current school name, so it stays in sync with
// whatever is set in Admin Settings at the time a student/teacher is created.
export const generateUsername = (fullName: string, schoolName: string): string => {
  const clean = fullName
    .toLowerCase()
    .replace(/[^a-z0-9 ]/g, "")
    .trim()
    .replace(/\s+/g, "");
  const domain = schoolName
    .toLowerCase()
    .replace(/[^a-z0-9 ]/g, "")
    .trim()
    .replace(/\s+/g, "");
  return `${clean}@${domain}.edu.ng`;
};
...
export const getInitialDatabase = (): Database => {
  ...
  for (let i = 0; i < 64; i++) {
    ...
    mockStudents.push({
      id,
      fullName: name,
      username: generateUsername(name, getInitialSchoolSettings().schoolName),
      regNo: regNum,
      classId: "SS 2",
      passportUrl: DEFAULT_STUDENT_AVATAR,
      session: session,
